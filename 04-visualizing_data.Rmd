...
**Compare shapes of plots instead of summary statistics? (Berkeley)**

**Most important thing with graphics is making it obvious for your audience to see the findings you want to get across**

**To create insights into the data**
...
```{r echo=FALSE, warning=FALSE, fig.height = 2, fig.cap="Strip Plot of Hourly Temperature Recordings from NYC in 2013"}
...
Note how the `facet_grid` function arguments are written here.  We are wanting the names of the airports vertically and the `carrier` listed horizontally.  As you may have guessed, this argument and other _formulas_ of this sort in R are in `y ~ x` order.  We will see more examples of this in Chapter \@ref(infer).
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Are the largest median arrival delays where you expected them to occur on the line-graph above in Figure \@ref(lineflights)?
